...
export default function LoginPage() {
  ...
  const redirectUrl = searchParams.get("redirect") || "/" // default redirect

  // Redirect if already logged in
  useEffect(() => {
    const token = localStorage.getItem("authToken")
    if (token) {
      router.replace(redirectUrl) // redirect to original page if logged in
    }
  }, [router, redirectUrl])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(redirectUrl) // redirect to original page after login
    } catch (err: any) {
      setError(err.message || "An unexpected error occurred")
    } finally {
      setLoading(false)
