...
export function NotesSection() {
  ...
  // [UPDATED] This function checks for a token and navigates
  const handleAccessMaterials = (slug: string) => {
    const token = localStorage.getItem("authToken")
    const destination = `/notes/${slug}`

    if (token) {
      // If user is logged in, go directly to the notes page
      router.push(destination)
    } else {
      // If user is NOT logged in, redirect to login
      // and pass the intended destination as a query parameter
          ...
          {subjects.map((subject, index) => (
            ...
            >
              ...
              <CardContent>
                ...
                {/* [UPDATED] This is no longer a <Link> */}
                <Button
                  size="sm"
                  className="w-full bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white border-0 transition-all duration-300"
                  onClick={() => handleAccessMaterials(subject.slug)}
                >
                  <BookOpen className="h-4 w-4 mr-1" />
                  Access Materials
                </Button>
